...
function buildAvailableItemsParams(availableItems) {
  if (!availableItems || !availableItems.lastUpdated) {
    throw new Error('lastUpdated is required and must come from client');
  }

  return [
    availableItems.productId,
    availableItems.itemsRemaining,
    normalizeClientDateTime(availableItems.lastUpdated)
  ];
}
...
const updateProducts_Batch = async (req, res) => {
  ...
  try {
    ...
    await connection.query(
        ...
        [availableItems.productId, availableItems.itemsRemaining, normalizeClientDateTime(availableItems.lastUpdated)]
      );
